$(function(){
    ...
    plugins_calendario_id = new Array;
    ...
    $.calendario({
        id:'input[data-calendario=data]'
    });
    $.calendario({
        id:'input[data-calendario=datahora]',
        hora:true
    });
     ...
     */
    $('.input_url_geral select, .input_url_geral input[type=url]').change(function(){
     ...
     */
    $('.input_booleano_geral').click(function(){
    ...
    /**
     * MUDA OS DADOS QUANDO FOR MOBILE
     */
    if(MOBILE == 1){
        // $('input[data-mascara=data]').attr('type', 'date');
        // $('input[data-mascara=datahora]').attr('type', 'datetime');
        // $('input[data-mascara=hora]').attr('type', 'time');
        // $('input[data-mascara=numero]').attr('type', 'number');
        // $('input[data-mascara=cep]').attr('type', 'number');
        // $('input[data-mascara=dinheiro]').attr('type', 'number');
        // $('input[data-mascara=cpf]').attr('type', 'number');
        // $('input[data-mascara=cnpj]').attr('type', 'number');
    }
});
...
(function($){
    ...
    $.textarea = function(options){
        ...
        if(settings.barra == 'completa'){
            ...
            var barra = 'undo redo | bold italic | removeformat | forecolor backcolor | blockquote | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link unlink | image media table | fullscreen'
        }else {
    ...
    $.calendario = function(options){
        ...
        if($('#plugins_calendario').size() > 0) $('#plugins_calendario').remove();

		$('body').append('<div id="plugins_calendario"><div id="plugins_calendario_header"><div class="plugins_calendario_botoes" id="plugins_calendario_mes_anterior" data-acao="mes_anterior"></div><div class="plugins_calendario_texto" id="plugins_calendario_mes_texto">JUNHO</div><div class="plugins_calendario_botoes" id="plugins_calendario_mes_proximo" data-acao="mes_proximo"></div><div class="plugins_calendario_botoes" id="plugins_calendario_ano_anterior" data-acao="ano_anterior"></div><div class="plugins_calendario_texto" id="plugins_calendario_ano_texto">2016</div><div class="plugins_calendario_botoes" id="plugins_calendario_ano_proximo" data-acao="ano_proximo"></div><div class="plugins_calendario_botoes" id="plugins_calendario_fechar"></div></div><ul class="plugins_calendario_coluna" id="plugins_calendario_semana"><li>'+getSemana(0,3)+'</li><li>'+getSemana(1,3)+'</li><li>'+getSemana(2,3)+'</li><li>'+getSemana(3,3)+'</li><li>'+getSemana(4,3)+'</li><li>'+getSemana(5,3)+'</li><li>'+getSemana(6,3)+'</li></ul><ul class="plugins_calendario_coluna" id="plugins_calendario_datas"></ul><div id="plugin_calendario_rodape"><input type="text" id="plugins_calendario_hora" onClick="this.select()" data-mascara="hora" value="" placeholder="00:00:00"><div id="plugins_calendario_ok">OK</div><div class="plugins_calendario_agora" id="plugins_calendario_hoje">HOJE</div><div class="plugins_calendario_agora" id="plugins_calendario_agora">AGORA</div></div></div>');
		...
		function getDataInicial(acao){
            ...
            if(plugins_calendario_id.indexOf(data_id) == -1) plugins_calendario_id.push(data_id);
			...
			if(acao == 'mes_anterior'){
				...
				var data_hora_explode = $(data_id).val().split(' ');
			...
			for(i=0;i<semana_primeira;i++){
				dias_mes_ultimo++;
				dias_lista += '<li class="plugins_calendario_dias_negativo">'+dias_mes_ultimo+'</li>';
			}
			// Datas do mes
			for(i=1;i<=dias_quantidade; i++){
                ...
                if(data_minimo > 0 && data_minimo > data_eua) dias_lista += '<li class="plugins_calendario_dias_negativo">'+i+'</li>';
                else if(data_maximo > 0 && data_maximo < data_eua) dias_lista += '<li class="plugins_calendario_dias_negativo">'+i+'</li>';
				else if(data == data_real) dias_lista += '<li class="plugins_calendario_dias plugins_calendario_dias_hover" data-dia="'+data_real+'">'+i+'</li>';
				else dias_lista += '<li class="plugins_calendario_dias" data-dia="'+data_real+'">'+i+'</li>';
			}

			// Datas negativas
			for(i=1;i<=semana_quantidade_a_mais; i++){
				dias_lista += '<li class="plugins_calendario_dias_negativo">'+i+'</li>';
			}
			...
			// Posiciona o bloco
			var bloco_width = parseFloat($(data_id).width()) + parseFloat($(data_id).css('border-left')) + parseFloat($(data_id).css('border-right'));
			var bloco_height = parseFloat($(data_id).height()) + parseFloat($(data_id).css('border-top')) + parseFloat($(data_id).css('border-bottom'));
		...
		$('body').on('focus', settings.id, function(){
            $('#plugins_calendario').show();
            ...
            if(minimo != 0){
                ...
                minimo = data_minimo.substring(4,8)+data_minimo.substring(2,4)+data_minimo.substring(0,2);
            }
            ...
            if(maximo != 0){
                ...
                maximo = data_maximo.substring(4,8)+data_maximo.substring(2,4)+data_maximo.substring(0,2);
            }
        ...
        $('body').on('keyup', settings.id, function(e){
            getDataInicial($($('#plugins_calendario').attr('data-id')).val());
        });

        $('body').on('click', '#plugins_calendario', function(e){
            e.stopPropagation();
        });
        $('body').click(function(){
            if($('#plugins_calendario').is(':visible')){
                $('#plugins_calendario').hide();
                return false;
            }
        });
		...
		$('body').on('click', '#plugins_calendario_mes_anterior, #plugins_calendario_mes_proximo, #plugins_calendario_ano_anterior, #plugins_calendario_ano_proximo', function(){
		...
		$('body').on('click', '#plugins_calendario_fechar', function(){
			if($('#plugins_calendario').is(':visible')) $('#plugins_calendario').hide();
		});
        // Da OK quando tiver data
		$('body').on('click', '#plugins_calendario_ok', function(){
            var input = $('#plugins_calendario').attr('data-id');
            var data = $('#plugins_calendario .plugins_calendario_dias_hover').attr('data-dia') || '';
		...
		$('body').on('click', '.plugins_calendario_dias', function(){
            ...
            $('#plugins_calendario .plugins_calendario_dias').removeClass('plugins_calendario_dias_hover');
        ...
        $('#plugins_calendario').on('click', '#plugins_calendario_hoje', function(){
            var data = new Date();
            var dia = data.getDate();
            var mes = data.getMonth();
            var ano = data.getFullYear();
            ...
            if(dia < 10) dia = '0'+dia;
            if(mes < 10) mes = '0'+mes;
            $($('#plugins_calendario').attr('data-id')).val(dia+'/'+mes+'/'+ano);
            $('#plugins_calendario').hide();
        });

        $('#plugins_calendario').on('click', '#plugins_calendario_agora', function(){
            var data = new Date();
            var dia = data.getDate();
            var mes = data.getMonth();
            var ano = data.getFullYear();
            ...
            $($('#plugins_calendario').attr('data-id')).val(dia+'/'+mes+'/'+ano+' '+hora+':'+minuto+':'+segundo);
            $('#plugins_calendario').hide();
        });
	}
    ...
    $.pagina = function(options){
        ...
        function pagina_abrir(botao, novo){
            $.loading('show');
            if(settings.scroll == false) $('body').css('overflow-y', 'hidden');

            if(novo == true){
                if($('#plugins_pagina_fechar').size() == 1) $('#plugins_pagina_fechar').stop().animate({opacity:0}, 500);
                ...
                .animate({
                    ...
                    $('#plugins_pagina_conteudo').load(botao.attr(settings.attr), function(){
                        $.loading('hide');
                ...
                $('#plugins_pagina_conteudo').load(botao.attr(settings.attr), function(){
                    $.loading('hide');
                    if(settings.fechar == true) $('#plugins_pagina_fechar').show().stop().animate({opacity:1}, 500);
                    $('#plugins_pagina_conteudo').show().stop()
                    .animate({
                        'margin-top':20,
                        opacity:1
                    }, 500);
        ...
        $('body').on('click', settings.botao, function(){
            if($('#plugins_pagina').size() == 1){
                ...
                $('#plugins_pagina_conteudo').css('max-width', settings.width);

                pagina_abrir($(this), false, 0);
            }
            return false;
        });
        ...
        if(settings.botao_fechar != ''){
            $('body').on('click', settings.botao_fechar, function(){
                fechar();
                return false;
            });
        }
        $('body').on('click', '#plugins_pagina, #plugins_pagina_fechar', function(){
            fechar();
            return false;
        });
     ...
     */
     $.alerta = function(options){
 		...
 		if(settings.texto == '') return false;

 		if($('#plugins_alerta_conteudo').size() > 0){
            fechar_alerta();
 			return false;
 		}
        ...
        if(settings.notificacao != ''){
            ...
            $('#plugins_alerta').append('<div id="plugins_alerta_conteudo"><div class="plugins_alerta_titulo">'+settings.titulo+'</div><div class="plugins_alerta_texto">'+settings.texto+'</div><div class="plugins_alerta_botao">CANCELAR</div><div id="'+settings.confirmar+'" class="plugins_alerta_botao">CONFIRMAR</div></div>');
            abrir_alerta();
        }else {
            $('body').append('<div id="plugins_alerta"></div>');
            $('#plugins_alerta').append('<div id="plugins_alerta_conteudo"><div class="plugins_alerta_titulo">'+settings.titulo+'</div><div class="plugins_alerta_texto">'+settings.texto+'</div><div class="plugins_alerta_botao">OK</div></div>');
            abrir_alerta();
        }
        ...
        function abrir_notificacao(){
            ...
            if($('#plugins_alerta_notificacao').size() == 1){
                $('#plugins_alerta_notificacao').stop().show()
     			.animate({top: -100, opacity:0}, 300, function(){
     				$('#plugins_alerta_notificacao').remove();
                    $('body').append('<div id="plugins_alerta_notificacao" class="'+settings.notificacao+'" data-temporizador="'+numero+'"><div class="plugins_alerta_texto">'+settings.texto+'</div><div class="plugins_alerta_botao">OK</div></div>');
                    $('#plugins_alerta_notificacao').stop().show()
            ...
            setTimeout(function(){
                if($('#plugins_alerta_notificacao[data-temporizador='+numero+']').size() == 1){
                    $('#plugins_alerta_notificacao[data-temporizador='+numero+']').stop().show()
         			.animate({top: -100, opacity:0}, 300, function(){
         				$(this).remove();
         			});
                }
        ...
        function abrir_alerta(){
     		$('#plugins_alerta').show().stop()
     		.animate({opacity:0}, 0)
            .animate({opacity:1}, 300);

     		$('#plugins_alerta_conteudo').show().stop()
     		.animate({top: -100, opacity:0}, 0)
     		.animate({top: 20, opacity:1}, 300);
